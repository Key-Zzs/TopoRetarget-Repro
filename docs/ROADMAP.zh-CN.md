...
21 关键点 source-hand adapter，以及阶段 4 的通用机器人手运动学接口与 Arti-MANO 目标手
适配。阶段 5–19 尚未开始，不能把规划内容描述为已实现算法。

长期阶段依次为：

0. 仓库创建与架构搭建；1. 论文忠实度审计；2. canonical HOI schema 与坐标约定；3. MANO
   到 MediaPipe 风格 21 点（有界完成，保留显式假设）；4. 通用 URDF/FK 与 Arti-MANO 目标手适配器（完成，保留显式假设）；5. GRAB 适配器；6. 物体表面采样、碰撞
