    ...
    if start >= 0:
        try:
            ...
            interaction_available = bool(
                data.get("interaction_provenance", {}).get("available", True)
            )
            if interaction_available:
                data_ok = data_ok and len(data.get("interaction_profiles", {})) >= max(
                    profiles - 1, 1
                )
            else:
                data_ok = data_ok and not data.get("interaction_profiles", {})
...
<h2>Frame metrics</h2><pre id="metrics"></pre><h2 id="diagnosticHeading" style="display:none">Diagnostic summary</h2><pre id="diagnostic" style="display:none"></pre><h2>Provenance</h2><div class="hint" id="provenance"></div>
</aside></main>
<script>
const DATA = __DATA__;
const canvas=document.getElementById('scene'),ctx=canvas.getContext('2d'),frameInput=document.getElementById('frame'),frameLabel=document.getElementById('frameLabel'),metricsBox=document.getElementById('metrics'),diagnosticBox=document.getElementById('diagnostic'),modeInput=document.getElementById('mode'),profileSelect=document.getElementById('profileSelect');
...
function drawObjectContext(index,w,h){const pose=object.poses[index];ctx.fillStyle='#64748b';ctx.globalAlpha=0.22;for(const p of object.vertices){const s=project(transformPoint(pose,p),w,h);ctx.fillRect(s[0]-1,s[1]-1,2,2)}ctx.globalAlpha=1}function drawObjectSurface(index,w,h){const pose=object.poses[index];drawMesh(object.vertices.map(p=>transformPoint(pose,p)),object.faces,'#dc2626',0.42,w,h)}
