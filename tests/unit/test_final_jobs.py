...
def test_explicit_resume_requires_a_quiescent_queue(tmp_path) -> None:
    root = tmp_path / "repo"
    pause_final_jobs(root, reason="unit test")
    resumed = resume_final_jobs(root, max_final_workers=1, reason="validated test resume")
    assert resumed["state"] == "READY"
    assert not paused(root)
...
def test_worker_claim_enforces_selected_cap_and_releases_own_slot(tmp_path) -> None:
    root = tmp_path / "repo"
    pause_final_jobs(root, reason="unit test")
    resume_final_jobs(root, max_final_workers=1, reason="validated test resume")
    claim_final_job(root, job_id="one", payload={"dataset": "unit"})
    with pytest.raises(FinalJobPaused, match="worker cap reached"):
        claim_final_job(root, job_id="two", payload={"dataset": "unit"})
    release_final_job(root, job_id="one")
    claim_final_job(root, job_id="two", payload={"dataset": "unit"})
